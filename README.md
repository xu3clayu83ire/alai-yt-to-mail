...
v3 起支援**無帳號訂閱模式**：使用者不需要註冊帳號，直接在首頁從管理員維護的頻道清單中選擇頻道並輸入 email 即可訂閱；可在「我的訂閱」頁面以 email 查詢並管理訂閱。當頻道連續 N 天無新影片時，排程器會自動取消訂閱並寄送通知信。

v3.1 起採用**頻道白名單機制**：管理員透過後台（`/admin/channels`）維護可訂閱的頻道清單；使用者訂閱時從下拉選單選擇頻道，無法自行輸入任意 YouTube 網址。
...
yt-to-mail/
  ...
  lambda/
    api/
      ...
      routers/            # 路由模組（auth / subscriptions / history / public / admin / admin_channels）
      models/             # Pydantic 資料模型
      services/           # 業務邏輯層
      dependencies.py     # JWT 依賴（get_current_user / get_current_admin）
  frontend/
    src/
      pages/              # 頁面元件（含 AdminLoginPage / AdminSubscriptionsPage / AdminChannelsPage / PublicSubscriptionListPage）
      api/                # API 呼叫模組（auth / subscriptions / public / admin / publicChannels / adminChannels）
...
| `CHANNELS_TABLE` | 是 | DynamoDB channels 表名稱（v3.1 新增） | `yt-to-mail-channels` |
...
| `yt-to-mail-channels` | `channel_id` (String) | — | 管理員維護的可訂閱頻道白名單（v3.1 新增） |
| `yt-to-mail-subscriptions` | `id` (String) | `user_id-index`（PK: user_id）、`channel_id-index`（PK: channel_id，v3.1 新增） | 頻道訂閱設定（含 `auto_cancel_days`、`no_new_video_days`） |
...
> v3.1 起，新增 `yt-to-mail-channels` 資料表，管理員可透過 `/admin/channels` 後台管理可訂閱的頻道清單。`yt-to-mail-subscriptions` 新增 `channel_id-index` GSI，供管理員刪除頻道時串聯查詢並取消相關訂閱。
...
| GET | `/public/channels` | 取得可訂閱頻道清單（v3.1 新增，前端下拉選單使用） |
...
| POST | `/admin/channels` | 新增可訂閱頻道（v3.1 新增） |
| GET | `/admin/channels` | 列出所有頻道（v3.1 新增） |
| PATCH | `/admin/channels/{channel_id}` | 更新頻道資訊（v3.1 新增） |
| DELETE | `/admin/channels/{channel_id}` | 刪除頻道，串聯取消所有訂閱並寄通知信（v3.1 新增） |
...
| 路徑 | 頁面元件 | 守衛類型 | 說明 |
|------|----------|----------|------|
| `/` | `AddSubscriptionPage` | 公開 | 訂閱頻道（無需帳號），從下拉選單選擇頻道，為首頁 |
...
| `/admin/channels` | `AdminChannelsPage` | `AdminRoute`（admin JWT） | 管理員後台：頻道白名單管理（v3.1 新增） |
| `*` | — | — | 未知路徑一律重導至 `/` |
